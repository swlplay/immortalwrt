...
SSH_CONVERT_SCRIPT="$SCRIPT_DIR/qhssh.sh"
...
# ==================== 执行 SSH 转换脚本 ====================
if [ -f "$SSH_CONVERT_SCRIPT" ]; then
    echo "执行 SSH 转换脚本: $SSH_CONVERT_SCRIPT"
    cd "$MAIN_REPO"
    bash "$SSH_CONVERT_SCRIPT"
    cd - > /dev/null
else
    echo "警告：SSH 转换脚本 $SSH_CONVERT_SCRIPT 不存在，跳过"
fi
...
if [ -z "$upstream_branch" ]; then
    ...
    if ! git diff --quiet || ! git diff --cached --quiet; then
        ...
        if git stash push -m "自动 stash 于 $(date)"; then
            stashed_main=true
        else
            echo "警告：stash 失败，继续执行（可能没有修改）"
        fi
    fi
